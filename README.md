...
#### 新功能
- **Spawn Subagent** - `spawn_subagent` 工具，工作区内临时执行子 Agent (#4806)
- **Open Directory** - Coding 模式新标签页，引用本地项目无需复制文件 (#4762)
- **腾讯元宝频道** - 新增腾讯元宝 Bot 频道集成 (#4856)
- **飞书话题回复** - Agent 可在飞书消息话题内回复 (#4708)
- **OpenRouter 归因** - 添加应用归因请求头，提升排名可见性 (#4809)
- **动态上传限制** - 上传文件大小限制可在控制台和频道间配置 (#4734)

#### 变更
- 飞书卡片系统重构，匹配企业微信/QQ 模式，修复流式模式下已解决卡片问题 (#4742)

#### Bug 修复
- Tauri 桌面应用捆绑 CLI (#4779)、隐藏 Windows git 控制台窗口 (#4696)
- 移动端聊天输入安全区域适配 (#4751)、聊天输入草稿清除 (#4755)
- OneBot WebSocket 连接丢失自动重启 (#4793)
- 推理内容跨文件块保留 (#4728)、消息时间戳支持用户时区 (#4763)
- 技能更新时保留标签/启禁用/频道配置 (#4847)
- QwenPaw Pet 自动安装缺失依赖，修复 v1.1.9 启动失败 (#4801)
...
| 控制 | 频道 | 启用/禁用频道、填入凭据、快速文档链接、飞书区域选择器（v0.2.0+）、QQ 即时确认（v1.1.3+）、频道健康检查 API（v1.1.3+）、统一访问控制（v1.1.9+）、飞书话题回复（v1.1.10+） |
| 控制 | 会话 | 筛选、重命名、删除会话 |
