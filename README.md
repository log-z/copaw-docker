...
#### 新功能
- **Browser Use 批量操作** - 单次工具调用执行多个浏览器步骤（导航、点击、输入、截图等）
- **Browser Use 文件下载** - 通过点击页面元素或获取 URL 下载文件
- **GPT Image 2 参考图片** - 支持上传 1-16 张参考图片
- **外部 Agent 异步执行** - `delegate_external_agent` 支持异步流式执行，含任务列表和状态查询
- **Shell 可执行文件配置** - 可选择运行命令的 Shell
- **远程 MCP OAuth 2.1** - 通过 OAuth 2.1 with PKCE 认证远程 MCP 服务器
- **控制台插件管理** - 从 Settings UI 通过本地路径、ZIP 上传或 URL 安装和卸载插件
- **Qwen-Image 和 Wan 2.7 插件** - 新增图像生成/编辑和视频生成插件
- **插件 HTTP API 路由** - 插件可注册自定义 HTTP API 端点
- **多文件附件** - 聊天输入支持单条消息选择多个文件
- **印尼语支持** - 完整 Bahasa Indonesia 本地化
- **收件箱 (Inbox)** - 集中管理工具审批和 Cron 执行结果
- **Cron 任务增强** - 一次性定时执行、日历视图、内置模板、执行历史和追踪
- **飞书语音气泡** - 音频文件渲染为原生语音气泡
- **钉钉引用消息** - 回复中包含引用文本和媒体
- **飞书二维码创建机器人** - 扫描二维码创建飞书/Lark 机器人
- **企业微信流式输出** - 可选流式回复，支持实时投递和推理指示器
- **模型选择器重设计** - 按提供商分组的可搜索平面列表

#### 性能优化
- FastAPI 模型和提供商依赖转为异步
- 文件读取大小限制从 1 GB 降低到 200 MB
- Keyring 操作新增 10 秒超时，防止启动阻塞

#### Bug 修复
- 火山引擎模型 ID 修正 (#4169)
- 流式解析器过滤空工具调用块 (#4234)
- 飞书 WebSocket 保活检测和恢复 (#4241)
- 会话历史路由修复 (#4203)
- Plan Panel 暗黑模式对比度改善 (#4190)
- 移动端侧边栏窄屏自动折叠 (#4225)
...
| 组 | 功能 | 说明 |
|----|------|------|
| 聊天 | 聊天 | 和 QwenPaw 对话、管理会话、切换模型、多模态支持、SSE 流式响应、音视频支持（v0.2.0+）、多模态预览（v1.0.0+）、频道标签（v1.0.0+）、命令建议（v1.0.0+）、选择 Agent 对话（v1.0.0.post1+）、聊天搜索（v1.0.2+）、置顶会话（v1.0.2+）、输入历史（v1.0.2+）、Mission 模式（v1.1.2+）、Plan 模式（v1.1.4+）、会话右键菜单（v1.1.4+）、多文件附件（v1.1.7+）、浮动聊天按钮（v1.1.7+） |
| 控制 | 频道 | 启用/禁用频道、填入凭据、快速文档链接、飞书区域选择器（v0.2.0+）、QQ 即时确认（v1.1.3+）、频道健康检查 API（v1.1.3+） |
| 控制 | 会话 | 筛选、重命名、删除会话 |
| 控制 | 定时任务 | 创建/编辑/删除任务、立即执行、日历视图（v1.1.7+）、一次性执行（v1.1.7+）、执行历史（v1.1.7+） |
| 智能体 | 工作区 | 编辑人设文件、查看记忆、上传/下载、代理选择器、标签页界面（v1.1.1+）、备份与恢复（v1.1.3+）、每 Agent 模型分配（v1.1.4+） |
| 智能体 | 技能 | 启用/禁用/创建/**导入**/AI优化/删除技能、安全扫描、Skill Pool 双层架构（v1.0.0+）、技能命令 `/<skill>` （v1.0.2+）、技能池标签（v1.0.2+）、技能选择改进（v1.1.1+）、技能导入中心（v1.1.2+）、技能页面重设计（v1.1.3+）、技能语言切换（v1.1.3+）、插件管理（v1.1.7+） |
...
| 设置 | 模型 | 配置提供商（含自定义提供商）、管理本地/Ollama/LM Studio 模型、选择模型、搜索过滤（v0.2.0+）、QwenPaw Local Model（v1.0.0+）、视频分析（v1.0.0.post1+）、`/model` 聊天命令（v1.0.2+）、模型 ID 自动补全（v1.1.1+）、模型选择器重设计（v1.1.7+） |
