...
#### 新功能
- **Tauri 2.x 桌面应用** - 基于 Tauri 的原生桌面应用，支持 macOS 和 Windows (#3813)
- **Coding 模式** - Web IDE 三栏布局：文件树、标签编辑器、Git 面板 (#4578, #4671)
- **技能市场** - 从多个提供商（阿里云、ClawHub、ModelScope）浏览和安装技能 (#4518, #4623)
- **OpenCode Go** - OpenCode 和 OpenCode Go 共享提供商，可切换 API 端点 (#4536, #4549)
- **浏览器无头模式警告** - 检测到验证页面时提示切换有头模式 (#4603)
- **后台任务超时** - `submit_to_agent` 新增 `task_timeout` 参数和 CLI `--task-timeout` (#4547)
- **文件块上下文处理** - 文件块与图片/音频/视频块统一处理和 Token 计数 (#4567)
- **工具拒绝反馈** - 用户拒绝工具调用后 Agent 不再重试 (#4569)
- **隔离 Cron 会话** - 非共享 Cron 任务使用清洁上下文执行 (#4602)
- **聊天输入草稿持久化** - 输入文本和光标位置跨页面导航保持 (#4598)
- **聊天历史记忆摘要** - 聊天历史包含记忆摘要 (#4658)
- **统一访问控制** - 每频道白名单/黑名单/待审批系统，含控制台 UI (#4565)
- **QQ 工具审批卡片** - QQ 频道交互式批准/拒绝卡片 (#4667)

#### 变更
- 插件管理器 UI 重设计：i18n 描述、类型过滤、名称搜索、自动重载 (#4545, #4588)
- 技能内容导入/保存前验证 (#4591)
- 控制台 UI 刷新：头部导航、环境与安全设置页面 (#4654, #4657)

#### Bug 修复
- 微信消息去重、过期 context_token 跳过、API 消息准确失败报告 (#4576, #4597, #4618, #4627)
- 钉钉中文文件名百分号解码 (#4600)、DM Webhook 键冲突修复 (#4665)
- MCP 客户端密钥路由修复 (#4560)
- QwenPaw Pet Windows 兼容性、连续对话卡 Done 状态修复、自动启动改为 opt-in (#4564, #4626)
- 暗黑模式 Plan Panel 和 Pet 导入区可见性修复 (#4570, #4599)
- Whisper 语音按钮闪烁修复 (#4601)
- 代理客户端 IP 真实解析修复 (#4562)
- Gemini `max_tokens` 映射为 `max_output_tokens` 修复 (#4621)
...
| 组 | 功能 | 说明 |
|----|------|------|
| 聊天 | 聊天 | 和 QwenPaw 对话、管理会话、切换模型、多模态支持、SSE 流式响应、音视频支持（v0.2.0+）、多模态预览（v1.0.0+）、频道标签（v1.0.0+）、命令建议（v1.0.0+）、选择 Agent 对话（v1.0.0.post1+）、聊天搜索（v1.0.2+）、置顶会话（v1.0.2+）、输入历史（v1.0.2+）、Mission 模式（v1.1.2+）、Plan 模式（v1.1.4+）、会话右键菜单（v1.1.4+）、多文件附件（v1.1.7+）、浮动聊天按钮（v1.1.7+）、固定会话抽屉（v1.1.8+）、`/make-skill` 命令（v1.1.8+）、聊天输入草稿持久化（v1.1.9+）、Coding 模式（v1.1.9+） |
| 控制 | 频道 | 启用/禁用频道、填入凭据、快速文档链接、飞书区域选择器（v0.2.0+）、QQ 即时确认（v1.1.3+）、频道健康检查 API（v1.1.3+）、统一访问控制（v1.1.9+） |
| 控制 | 会话 | 筛选、重命名、删除会话 |
...
| 智能体 | 技能 | 启用/禁用/创建/**导入**/AI优化/删除技能、安全扫描、Skill Pool 双层架构（v1.0.0+）、技能命令 `/<skill>` （v1.0.2+）、技能池标签（v1.0.2+）、技能选择改进（v1.1.1+）、技能导入中心（v1.1.2+）、技能页面重设计（v1.1.3+）、技能语言切换（v1.1.3+）、插件管理（v1.1.7+）、官方插件分发（v1.1.8+）、技能市场（v1.1.9+） |
