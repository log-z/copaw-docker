...
> 完整 changelog 范围为 v2.1.0...v2.2.0。

#### 新功能

**QwenPaw Hub**
- **自托管多用户 Hub** - 使用本地进程或 Docker 运行时自托管多用户 QwenPaw Hub，含工作区级访问控制、凭据管理和反向代理支持 (#7112)

**QwenPaw Data**
- **原生 PawApp 部署** - 作为原生 PawApp 部署 QwenPaw Data，含持久分析工作区、打包运行时和一键 Docker Compose 示例 (#6940, #7190)

**QwenPaw Mail**
- **智能邮件助手** - 提供实时监控、搜索、会话感知操作、发件人允许/拒绝规则，以及对未知收件人的人工审批 (#6800)
- **桌面捆绑 Mail MCP** - 桌面应用捆绑并启动 Mail MCP (#7170)
- **中文可读** - 在中文对话中保持 Mail 工具响应可读 (#7303)

**模型与提供商**
- **统一模型选择** - 通过统一提供商发现、能力感知路由、回退策略和思考控制，使模型设置和选择更一致 (#6302)
- **新增提供商** - 新增 Volcengine Agent Plan 和 MiMo V2.5 提供商，刷新 Volcengine 目录 (#6515)
- **提示缓存观测** - Token Usage 新增提示缓存的命中与写入可观测性 (#7342)

**QwenPaw Creator**
- **Creator 1.1.0** - 扩展主流提供商的图片/视频生成，并通过将规划好的对话带入生成提示改进叙事视频 (#7167)
- **Creator 1.1.1** - 新增录制网站/桌面操作、百炼 Wan3 视频生成，以及可配置的自动视频质检 (#7274)

**控制台与工作区**
- **子 Agent 分组** - 将子 Agent 对话分组，并展示完成与未读状态 (#7035, #7275)
- **后台任务跟踪** - 通过紧凑任务列表和一致运行跟踪，更易跟踪后台工作 (#7083, #7319)
- **响应产物与媒体下载** - 展示响应产物，生成媒体可直接下载 (#7161, #7036)
- **系统提示文件选择** - 从工作区 UI 选择系统提示文件 (#7078)
- **工作区 UI 增强** - 创建工作区文件夹、复制文件 (#7151, #7314)
- **会话级项目目录** - 项目目录限定到各会话，使并发工作可安全使用不同根目录 (#6976)
- **Token Usage 扩展** - 新增模型、工具调用趋势，每 Agent 明细和全 Agent 视图 (#7207, #7219)
- **OS dock 自动隐藏** - 最大化窗口时自动隐藏 QwenPaw OS dock (#7233)
- **编辑器标签溢出导航** - 改进编辑器标签溢出时的导航 (#6983)
- **复制响应** - 复制助手响应时不包含隐藏推理文本 (#7448)

**技能、频道与媒体**
- **AnySearch 集成** - 集成 AnySearch 作为网页 SearchProvider 和 MCP 服务 (#7081)
- **技能搜索与批量操作** - 技能可搜索、批量启用/禁用 (#7140, #6990)
- **技能加载增强** - 更快的技能文件加载、更干净的绑定，内置技能自动更新 (#7097, #7232)
- **统一市场** - 应用、插件和技能在同一市场管理 (#6880, #7017, #7235)
- **PawApp 免重载打开** - 新安装的 PawApp 无需重载控制台即可打开 (#7238)
- **CLI 交互配置插件频道** - 从 CLI 交互式配置插件频道 (#6943)
- **自定义频道纳入 Agent MCP 规则** - 自定义频道纳入 Agent MCP 访问规则 (#7225)
- **Matrix 群组隔离** - Matrix 群组会话和记忆按发送者隔离，对瞬时登录失败重试 (#7001, #7109)
- **钉钉群组共享上下文** - 配置后钉钉群组可共享会话上下文；可配置宽屏卡片自动布局 (#7208, #7416)
- **OneBot 媒体本地下载** - OneBot 媒体在处理前本地下载 (#6715)
- **Computer-use 观察相关窗口** - Computer-use 可观察相关窗口 (#7037)
- **超大图片缩放** - 发送到模型前可缩放超大图片 (#7294)
- **控制台停止传播** - 控制台停止请求传播到活跃 Agent 工具 (#7349)
- **助手消息自动折叠** - 完成的助手进程消息自动折叠 (#7374)

#### 变更

- **并发启动 Drivers** - 持久 Drivers 并发启动，加快工作区启动 (#7174)
- **流式开销降低** - 长响应期间降低流式开销 (#7244, #7239)
- **提示缓存前缀稳定** - 稳定跨请求的可复用提示缓存前缀 (#7346)
- **长 Markdown 聊天保持响应** - 长且含大量 Markdown 的聊天保持响应 (#7176)
- **Markdown 列表间距** - Markdown 列表使用更紧凑一致的间距 (#7317)
- **ReMe 改进** - 改进 ReMe 运行时仪表盘和长期记忆指导 (#6984, #6997)
- **通知抑制** - 定时任务未变化时抑制通知 (#7115)
- **移除过时提醒** - 移除过时的 Desktop Mode 提醒和过时审批提示 (#7245, #6981)
- **AgentScope 更新** - 更新 AgentScope 至 2.0.7.post1 (#7276, #7373)
- **控制台聊天包刷新** - 刷新本版本使用的控制台聊天包 (#7257, #7289)
- **审批模式说明** - 澄清各审批模式行为 (#7368)
- **嵌入模型变更显式化** - 使嵌入模型变更显式且可逆，作用域重索引成功前回退 BM25 (#7133)
- **市场更名** - Marketplace 更名为 Extensions，并恢复控制台暗黑模式样式 (#7454)

#### Bug 修复

**模型、运行时与记忆**
- **恢复免费模型列表** - 恢复完整的免费模型列表和旧模型名称 (#7175)
- **刷新模型目录** - 刷新阿里云和 Kimi 模型目录 (#7271, #7277)
- **输出能力与请求限制分离** - 分离模型输出能力与请求限制，使路由和校验使用正确元数据 (#7337)
- **DashScope 兼容** - 改进与严格 DashScope 工具 schema 和音频消息的兼容 (#7284, #7024)
- **流式重试限制** - 遵守服务端提供的流式重试限制 (#6617)
- **卡住的模型流恢复** - 从卡住的模型流恢复 (#7150)
- **模型超时报告** - 准确传播并报告模型超时 (#7268, #7308)
- **AgentScope 2.0.6 兼容** - 保持 Agent 行为与 AgentScope 2.0.6 兼容 (#6908)
- **Ollama 嵌入后端** - 启用 Ollama 嵌入后端 (#7131)
- **保持嵌入验证** - 导航过程中保持嵌入验证 (#7227)
- **标题排除推理文本** - 生成聊天标题时排除推理文本 (#7187)
- **后台任务默认超时** - 为后台任务提供可靠默认超时而不覆盖显式值 (#6869)
- **ReMe 服务提前启动** - 在模型配置前启动 ReMe 记忆服务，使本地记忆功能无需配置模型即可用 (#7468)
- **~ 路径纳入统计** - 将基于 `~` 的 Agent 工作区路径纳入模型和工具用量趋势 (#7432)

**控制台、文件与媒体**
- **历史会话重开** - 直接从新聊天重开唯一的最近会话 (#7104)
- **助手完成时间** - 显示助手响应实际完成时间 (#6938)
- **历史 data-URL 图片渲染** - 重载后渲染历史 data-URL 图片 (#7069)
- **模型选择器样式** - 改进模型选择器样式和暗黑模式会话分组 (#7137, #7111)
- **隐藏空徽章与控制** - 隐藏空 grep 徽章和不可用审批控件 (#7165)
- **移动端输入控件** - 改进移动端输入控件 (#7334)
- **cron 日历对齐** - cron 日历日期与周日起首标头对齐 (#7352)
- **切换 Agent 保持嵌入校验** - 切换 Agent 时保持嵌入校验 (#7353)
- **保留文件名** - 保留上传和非 ASCII 文件名 (#6492, #7192)
- **CRLF 与预览刷新** - 保留 Windows CRLF 换行并在变更后刷新文件预览 (#7091, #7094)
- **远程文件下载回退** - 下载器超时后继续远程文件下载回退 (#6371)
- **远程图片持久化** - 保存工具结果前持久化远程图片 (#7146)
- **工具结果视频投递** - 通过 OpenAI Responses API 投递工具结果视频 (#7061)
- **内联视频限制可配置** - 使内联视频限制可配置 (#7071)
- **保留本地文件 URL** - 规范化媒体消息时保留本地文件 URL (#7343)
- **截图保存目录** - 将截图保存到活跃项目目录 (#7439)
- **Daily Paper 文档链接** - 将 Daily Paper 链接到正确的 QwenPaw 文档 (#7466)

**频道**
- **元宝消息解码** - 用 protobuf 6 及更高版本恢复元宝消息解码 (#7421)

**插件、桌面与运维**
- **插件工作区状态恢复** - 重载前恢复插件工作区状态，并隔离插件命名空间间的绝对导入 (#6996, #6688)
- **Windows WebView2 恢复** - 浏览器崩溃后恢复 Windows WebView2 (#7128)
- **Windows 安装器匹配** - 避免 Windows 安装器中的错误进程匹配，包括 NSIS 调用者 (#7281, #7323)
- **NSIS 卸载处理** - 处理 NSIS 卸载阻塞，并显式设置应用数据清理选项 (#7336, #7354)
- **工作区启动取消清理** - 工作区启动被取消时安全清理 (#7194)
- **环境配置保护** - 保护环境配置免受部分或损坏写入 (#7135)
- **SSE 关闭后备份任务** - SSE 连接关闭后保持备份任务运行 (#7283)
- **Agent 配置一致性** - 跨共享文件系统和并发写入保持 Agent 配置持久化一致 (#6767)
- **强制 File Guard 路径** - 活跃策略评估期间强制 File Guard 路径 (#7375)
- **沙箱挂载路径扩展** - 正确扩展沙箱挂载路径 (#7116)
- **防病毒干扰** - 沙箱设置期间避免杀毒软件干扰 (#6986)
- **QQ 会话隔离** - 隔离 QQ 会话并保留回复路由 (#7169)
- **小艺主动发送失败报告** - 可靠报告小艺主动发送失败并保留会话 ID (#7147)
- **依赖安全补丁** - 修补安全敏感的网站和 Creator 依赖 (#7172)
- **插件加载提速** - 通过避免每个插件后全模块扫描加速插件加载 (#7383)
- **恢复 ReMe 插件** - 在打包桌面构建中恢复 ReMe 及其 Auto Fin 和 Daily Paper 插件 (#7458)
- **防止 Python 运行时混合** - 防止桌面包混合不同 Python 运行时源 (#7372)
- **shell 续行防护** - 阻止 shell 续行变体绕过敏感路径保护 (#7472)

**新贡献者**: cyruszhang、Luohh5、LUOSENGWA、An-idd、TinyBai、c020627、suantea、xiaoka76、anysearch-ai、guodaxia103、kilowu、kaiwangleo。

---

### v2.1.0 (2026-08-13)

> **重大版本**，完整 changelog 范围为 v2.0.1...v2.1.0。

#### 新功能

**QwenPaw OS Shell**
- **桌面窗口化应用** - 在可移动、可缩放窗口中打开应用，含启动器、任务栏、通知和已保存布局 (#6645)
- **统一应用目录** - 已安装应用与市场应用在 App Center 和 QwenPaw OS Shell 之间共享同一目录 (#6718)

**统一文件工作区**
- **聊天内文件管理** - 无需离开聊天即可浏览、预览、编辑、比较、上传和下载工作区文件 (#6504)
- **OS Shell 直开文件** - 直接从 QwenPaw OS Shell 打开 Files 工作区 (#6758)
- **Robot Framework 语法高亮** - 编辑 Robot Framework 文件时支持语法高亮 (#6519)

**QwenPaw Creator**
- **视频创作** - 将创意或现有素材转化为完整视频，由一组专门 Agent 协作完成 (#6284)
- **聚焦编辑** - 选择片段、字幕、特效、素材或文本进行定向 Agent 编辑，同时保留直接控制 (#6740)
- **多模式创作** - 支持图片、视频、语音、字幕、运动、叠加层和并行生产步骤 (#6740)
- **媒体任务恢复** - 新增媒体任务恢复、渲染修复、结构化日志和评审反馈 (#6556, #6641)
- **设置中心与外置技能** - 新增设置中心、外部 Skills、异步媒体生成、更多渲染格式，改进 Windows 和窄屏支持 (#6870)

**跨 Harness 集成**
- **Codex/Qoder Agent** - 创建由 Codex 或 Qoder 驱动的 Agent，支持按会话模型、权限、Skills 和 MCP 服务 (#6397)
- **ACP 临时凭据** - ACP 客户端可传递临时模型、项目文件夹和 MCP 服务，无需保存运行时凭据 (#6754)
- **ACP 模型报告** - ACP 会话现向兼容客户端报告可用模型和当前模型 (#6531)

**Browser-use**
- **Beta 阶段提醒** - Browser-use 仍处于 Beta：可使用独立浏览器会话或连接用户已登录 Chrome 的选定标签页。浏览器侧代码执行不受主机级沙箱保护，操作策略尚未强制 (#6276, #6157)

**Computer-use**
- **Windows 桌面应用自动化** - 新增 Windows 桌面应用自动化。macOS 打包和签名已完成，设备级端到端验证仍在进行中。操作需按应用审批，用户接管输入时停止 (#6424)
- **原生 Computer-use 增强** - 新增键盘序列，改进 Windows 模态定位和文本编辑，macOS 菜单、拖拽、捕获和焦点更可靠 (#6891, #6913, #6977)

**上下文、恢复与长时任务**
- **会话快照与恢复** - 快照并恢复会话、记忆和选定的工作区文件，不改变项目 Git 历史 (#6269)
- **长会话归档** - 长对话保留活跃任务，更早的精确历史移入可搜索索引 (#6323)
- **历史搜索增强** - 历史搜索返回配对的请求与响应，并理解基于日期的查询 (#6237)
- **Visual Compact** - 对支持的多模态模型缩减长上下文输入，同时保留源恢复 (#6456)
- **后台长时工具** - 长时运行工具可在后台继续，支持进度、延展和取消控制 (#6151)
- **工具工作流编排** - 工具工作流可复用结果、分支、循环，并并行运行独立步骤 (#5698)
- **ReMe Light 重排序** - 记忆搜索可选通过 OpenAI 兼容端点对候选重排序，重排序不可用时优雅回退 (#6398)
- **ReMe Light 配置增强** - 新增验证过的可热更新嵌入设置、Daily Paper 和服务级调度、每 Agent 运行时状态、手动重建索引控制，以及独立的搜索和通知选项 (#6772)

**控制台与模型**
- **Agent 统计** - Agent 统计现显示 Token 用量，并将结果收窄到当前 Agent (#6503, #6402, #6862)
- **隐藏文件夹选择** - 项目目录选择可按需包含隐藏文件夹 (#6878)
- **代码块统一设计** - 代码块共享响应式、主题感知设计，含语法高亮、复制和下载操作；LaTeX 和 Mermaid 块新增本地化预览和源码标签页 (#6911)
- **记忆 3D 图** - 在交互式 3D 图中探索记忆关系，支持聚焦、过滤和减弱动效 (#6922)

**频道**
- **自定义网关端点** - 飞书、QQ、企业微信、小艺和元宝可使用可选的网关端点用于私有部署和本地集成测试，不改变默认值 (#6907)
- **Bot 身份冲突警告** - 保存已启用的内置频道时，若同一 Bot 身份已被其他运行中的 Agent 使用则发出警告，同时允许显式覆盖 (#6909)

#### 变更

- **App Center 拆分** - App Center 分离已安装与可用应用，扩展应用详情，新增简单模式入口 (#6553, #6638)
- **Loop 模式对齐** - 聊天保持 Loop 模式与活跃 Agent 对齐，并列出本地化的 Oh-My-Paw 模式 (#6711, #6521)
- **斜杠命令技能** - 斜杠命令技能显示注入内容，跳过重复副本，可直接导入 QwenPaw Skill 链接 (#6345, #6650, #6517)
- **Windows 沙箱选择** - Windows 根据当前权限选择沙箱方式；各平台上报不支持的受限方式 (#6383, #6657)
- **OneBot/QQ 消息顺序** - 保留文本和媒体顺序，展开引用或转发的消息 (#6543, #6769)
- **OS Shell 模式切换** - Simple/Desktop 模式开关并排显示，缩短桌面模式标签 (#6763)
- **OS Shell 启动入口** - 统一为单一浏览器入口，从桌面内启动应用，移除过时的 Coding 启动路径 (#6868)
- **主动模式优化** - 主动模式复用活跃工作区，禁用时释放资源，使用轻量级 Web 工具进行研究 (#6712)
- **MiniMax 模型更新** - 为 M3 和 M2.7 代更新 MiniMax 模型和上下文限制 (#6479, #6554)
- **插件安装容错** - 插件安装对声明更新兼容上限的包更宽容 (#6532)

#### Bug 修复

**QwenPaw Creator**
- **失败工作恢复** - 输入变化时失败的工作恢复，避免重复视频渲染，最终合成前审查过时场景 (#6937)

**桌面**
- **macOS 签名身份复用** - Computer-use 复用已签名的 macOS 应用身份，跨启动保持权限 (#6590)
- **启动与关机处理** - 桌面启动处理端口错误；Windows 关机检查有界且保持隐藏 (#6705, #6203)
- **Chrome 重连** - Chrome 中断后重连，桌面找到打包浏览器，Windows 恢复锁防止并发恢复 (#6669)
- **OS Shell 窗口** - 窗口保持菜单栏可见、位于视口内，并允许文本选择和复制 (#6801, #6861)

**运行时与 Agent**
- **工具调用字段保留** - 提供商特定的工具调用字段在流式、历史存储、压缩或重放中不再丢失 (#6759)
- **上下文限制自动压缩** - 模型报告上下文限制错误时 Scroll 压缩上下文并重试一次；历史迁移保留会话身份 (#6267, #6068)
- **Codex Agent 管理** - Codex 运行时不可用时，Codex 驱动的 Agent 仍可管理 (#6664)
- **子 Agent 审批继承** - 子 Agent 继承当前会话审批级别；空批处理字段不再把单个任务变成批处理请求 (#6508, #6595)
- **Loop 限制回写** - 旧配置的 Loop 限制回写，重命名的每工具条目持久保留 (#6530, #6682)
- **Shell 执行** - Shell 执行保留多行命令，不再在继承的后台管道上挂起 (#6412, #6748)
- **无头任务与提交** - 无头任务构造有效用户消息；工作区提交在未配置时使用本地默认 Git 身份 (#6616, #6350)
- **复制 Agent** - 复制 Agent 不再创建空的 Skills 或定时任务脚手架 (#6493)
- **后台子 Agent 任务** - 后台子 Agent 任务等待权威 fork 终结，报告失败时不丢弃工作树变更 (#6725)
- **配置损坏恢复** - 损坏的 Agent 配置和无效 JSON 产生可恢复的配置错误，而非破坏配置加载 (#6615)
- **Scroll 召回** - Scroll 召回对子串和多词搜索找到完整 CJK 轮次，并拒绝反转的扩展范围 (#6824)
- **遗留会话媒体** - 遗留会话再次加载本地路径媒体，包括 Windows UNC 路径和百分号编码的非 ASCII 路径 (#6873)
- **记忆提示与标题** - 记忆提示不再破坏对话；普通非流式响应可生成聊天标题 (#6360, #6816)
- **继续摘要** - 继续摘要尊重禁用的思考，并保留中断或超时行为 (#6818)
- **无头 ACP 会话** - 无头 ACP 会话尊重临时模型上下文和输出限制而不保存 (#6933)

**控制台与 UI**
- **会话恢复** - 重连和 Agent 切换不再恢复错误的聊天、过时频道或重复的待处理消息 (#6602, #6546, #6382)
- **富消息与预览** - 富消息进入发送队列；多文件预览在编辑器中换行 (#6798, #6662)
- **工具输出显示** - 大型工具输出使用有界预览；长命令在卡片内换行 (#6637, #6677)
- **工具控件隐藏** - 工具控件在流式完成前保持隐藏 (#6749)
- **侧边栏折叠** - 侧边栏折叠时设置保持可见 (#6488)
- **生产构建 CSS** - 生产控制台构建不再遗漏依赖 CSS (#6639)
- **滚动稳定性** - 聊天滚轮滚动保持稳定，加载更多 spinner 不再离屏运行 (#6904, #6896)
- **头部菜单** - 头部资源菜单不再显示重复的 GitHub 链接 (#6903)
- **工作区预览** - 支持 Unicode PDF 文件名和 SVG 媒体类型，修正预览和编辑界面的暗黑模式样式 (#6915)
- **换行保留** - 长多行助手和工具输出保留换行；宽代码块和表格在容器内滚动 (#6890)
- **输入框回退** - 聊天回退到标准 textarea 输入；桌面和端到端验证支持 textarea 和 contenteditable 输入 (#6934)
- **上下文用量修正** - 图片上传不再将 Base64 数据计为文本而夸大上下文用量环 (#6968)
- **截图预览** - 大截图可预览，完成的工具不再保持旋转状态图标 (#6965, #6967)
- **每日记忆排序** - 每日记忆笔记按路径日期分组和排序 (#6941)

**治理与安全**
- **浏览器调试认证** - 浏览器调试要求认证，除非用户明确选择退出 (#6500)
- **项目导入路径** - 项目导入只读取已批准的路径，并报告跳过的敏感文件夹 (#6487, #6713)
- **沙箱清理** - 沙箱清理处理中断的会话；无效设置回退而不崩溃并显示警告 (#6600, #6582, #6747, #6857)
- **PYTHONHOME 隔离** - 沙箱化子进程不再继承 `PYTHONHOME`，防止解析错误的 Python 运行时 (#6902)
- **日志脱敏** - 对话命令参数不再写入日志 (#6692)

**记忆**
- **迁移保存修复** - 修复上下文迁移和 Web 工作区恢复期间的记忆与检查点保存 (#6592, #6597)
- **自动记忆进度** - 自动记忆进度在上下文压缩和会话恢复后保留；失败的提交可重试且不污染聊天历史 (#6830)

**MCP**
- **会话恢复** - 终止的 MCP 会话恢复时不再丢弃活跃 Agent 的每个工具 (#6894)

**CLI 与频道**
- **空闲任务触发** - 服务空闲时定时任务仍触发 (#6481)
- **暂停恢复保状态** - 暂停和恢复定时任务保留其启用状态 (#6691)
- **投递行为遵循** - 定时任务结果遵循配置的投递行为，包括迁移任务 (#6182, #6511)
- **缺失任务提示** - 暂停或恢复缺失的定时任务返回 not-found 响应 (#6858)
- **任务命令健壮性** - 任务命令报告缺失的 Agent 配置而不打印 traceback (#6865)
- **CLI 项目路径** - CLI 项目路径在全局选项后接受 (#6892)
- **预发布更新发现** - `qwenpaw update --prerelease` 现从 PyPI 发现更新的预发布版本 (#6939)
- **OneBot 安全默认** - OneBot 默认仅本地监听，暴露到网络时要求 token，配置声明与配置 API 对齐 (#6676, #6864)
- **频道审批路由** - 频道审批保留驱动门的路由字段；小艺消息隐藏内部 Scroll 标题 (#6833, #6888)
- **钉钉组织审批** - 钉钉组织审批将返回的凭据填入频道设置 (#6709)
- **Matrix Python 3.12** - Matrix 在 Python 3.12 上可连接并解密消息 (#6486)
- **时区解释一致** - 无时区的时间戳一致解释为本地进程时间 (#6685, #6855)

**其他**
- **备份恢复** - 备份恢复等待后台操作完成，接受零字节锁文件 (#6735, #6703)
- **OpenRouter** - 保留已知的多模态支持，重试可恢复的流式或推理错误 (#6733, #6714, #6721)
- **视频与音频** - 视频输入转发到声明视频支持的模型；频道音频再次转录 (#6495, #6573)
- **本地图片** - 本地图片在文件变化间保持原始内容；临时媒体错误不再禁用多模态支持 (#6930)
...
├── local_models/            # 本地模型目录（bin/logs/models/tmp，v2.0.0+）
├── models/                  # 本地模型数据目录
